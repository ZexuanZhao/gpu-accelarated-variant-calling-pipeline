...
rule all:
    input:
        multiqc_report =os.path.join(config["outdir"],"qc", "multiqc", "multiqc_report.html"),
        ...
        biallelic_snp = os.path.join(config["outdir"],"filtered_vcf","biallelic_snp.vcf.gz"),
        het = os.path.join(config["outdir"], "qc", "vcftools", "heterozygosity.het")
    shell:
        """
        echo "Job done!"
